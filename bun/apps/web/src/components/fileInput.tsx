...
const FileUpload = ({
  ...
  /**
   * The file upload mode.
   */
  uploadMode?: "single" | "multi";
  /**
   * The text displayed in the upload area when no files are selected.
   */
  defaultText?: string;
  /**
   * Additional text displayed below the default text.
   */
  otherText?: string;
  /**
   * The maximum allowed file size in bytes. Files larger than this will be rejected.
   */
  maxSize?: number;
  /**
   * An object specifying the accepted file types. The keys are MIME types, and the values are arrays of corresponding file extensions.
   */
  acceptedFileTypes?: Record<string, string[]>;
  /**
   * A callback function that is called when files are uploaded or removed.
   */
  onFilesUploaded: (
    files: (File & {
      preview: string;
  ...
  /**
   * A Zod schema for file validation.
   */
  zodSchema?: z.ZodMiniType;
  /**
   * External errors to display.
   */
   ...
   *
   * @param acceptedFiles The files dropped into the component
   */
  ...
  const renderFileList = (): JSX.Element => (
    <div className="mt-4 space-y-2">
      {files.map((file, index) => (
        <div
          key={index}
          className="flex items-center justify-between p-3 bg-secondary rounded-md border shadow"
        >
          ...
          <Button variant="ghost-fg" size="sm" onClick={() => removeFile(file)}>
